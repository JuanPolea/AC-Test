...
package com.jfmr.ac.test.data.remote.network.paging
...
import com.jfmr.ac.test.data.remote.network.API_PAGE
...
class RickAndMortyRemoteMediator @Inject constructor(
    ...
    private fun CharactersResponse.toDomain() = DomainCharacters(results = results.toDomain(), info = characterInfo?.toDomain())

    private fun List<com.jfmr.ac.test.data.open.rickandmorty.character.model.Character?>?.toDomain() = this?.filterNotNull()?.map { it.toDomain() }

    private fun com.jfmr.ac.test.data.open.rickandmorty.character.model.Character.toDomain() = id?.let {
    ...
    private fun com.jfmr.ac.test.data.open.rickandmorty.character.model.CharacterInfo.toDomain() =
